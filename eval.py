...
from src.utils import UnetEval, TempLoss, dice_coeff, fwiou
...
#     if hasattr(config, "multiclass") and config.multiclass or hasattr(config, "dataset") and config.dataset != "ISBI":
    ...
    model = Model(net, loss_fn=TempLoss(), metrics={"FWIou": fwiou(show_eval=config.show_eval)})

    print("============== Starting Evaluating ============")
    eval_score = model.eval(valid_dataset, dataset_sink_mode=False)["FWIou"]
    print("============== Cross valid FWIou is:", eval_score[0])
