...
from src.utils import StepLossTimeMonitor, UnetEval, TempLoss, apply_eval, filter_checkpoint_parameter_by_list, dice_coeff, fwiou
...
#     if hasattr(config, "multiclass") and config.multiclass or hasattr(config, "dataset") and config.dataset != "ISBI":
    ...
    print("train_dataset length is:", train_data_size)
    valid_dataset_size = valid_dataset.get_dataset_size()
    print("valid_dataset_size length is:", valid_dataset_size)
    ...
    if config.run_eval:
        eval_model = Model(UnetEval(net, need_slice=need_slice, eval_activate=config.eval_activate.lower()),
                           loss_fn=TempLoss(), metrics={"FWIOU": fwiou(False, config.show_eval)})
