...
#     eval_score = model.eval(dataset, dataset_sink_mode=False)["dice_coeff"][index]
    eval_score = model.eval(dataset, dataset_sink_mode=False)[metrics_name][0]
    return eval_score


class fwiou(nn.Metric):
    """Unet Metric, return fwiou and IOU."""
    ...
    def update(self, *inputs):
        ...
        inter = np.dot(y_pred.flatten(), y.flatten())  # TP
        union = np.dot(y_pred.flatten(), y_pred.flatten()) + np.dot(y.flatten(), y.flatten()) # TP+FN  TP + FP
        # TP: inter  2TP + FN + FP:union
        single_dice_coeff = 2 * float(inter) / float(union + 1e-6)
        single_iou = single_dice_coeff / (2 - single_dice_coeff)  # IOU = dice/(2-dice)
        
        # FWIOU =     [(TP+FN)/(TP+FP+TN+FN)] *[TP / (TP + FP + FN)]
        single_fwiou = np.dot(y.flatten(), y.flatten()) / len(y.flatten()) * single_iou
        
        if self.print_res:
            print("single fwiou is: {}, IOU is: {}".format(single_fwiou, single_iou))
        self._dice_coeff_sum += single_fwiou
...
class dice_coeff(nn.Metric):
    ...
    def update(self, *inputs):
        ...
        print('inter', inter)
        print('sum', np.sum(y_pred.flatten()))
        print('union', union)
        single_dice_coeff = 2 * float(inter) / float(union + 1e-6)
